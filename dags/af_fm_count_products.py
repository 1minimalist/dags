...
def count_products():
    ...
    cur = conn.cursor()
    cur.execute('delete from dar_group.fm_count_products where date = current_date')
    conn.commit()

    cur.execute("""insert into dar_group.fm_count_products
                    select *\
                    from(\
                    ...
                    from(\
                    select distinct date, ord, base, category[1] as category_1, category[2] as category_2, category[3] as category_3, category[4] as category_4, category[5] as category_5, sku_id\
                    from(\
                    select 1 as ord,'Кол-во товаров в ЕБТ'::text as base, count(distinct id) as sku_id, regexp_split_to_array(replace(replace(categories_array,'{',''),'}',''),',') as category, current_date as date\
                    from dar_group.darbazar_products\
                    where scope_ = '{darbazar}' and created_on >= '2018-10-01'\
                    group by categories_array\
                    union\
                    select 2 as ord, 'Кол-во всех товаров в номенклатуре'::text as base, count(distinct product_id) as sku_id, regexp_split_to_array(replace(replace(categories_array,'{',''),'}',''),',') as category, current_date as date\
                    from dar_group.fm_nomenclature\
                    where sale_channels = 'fortemarket'\
                    group by categories_array\
                    union\
                    select 3 as ord, 'Кол-во товаров в номенклатуре'::text as base, count(distinct product_id) as sku_id, regexp_split_to_array(replace(replace(categories_array,'{',''),'}',''),',') as category, current_date as date\
                    from dar_group.fm_nomenclature\
                    where sale_channels = 'fortemarket' and available is true\
                    group by categories_array\
                    union\
                    select 4 as ord, 'Кол-во активных товаров номенклатур'::text as base, count(distinct product_id) as sku_id, regexp_split_to_array(replace(replace(categories_array,'{',''),'}',''),',') as category, current_date as date\
                    from dar_group.fm_nomenclature\
                    where sale_channels = 'fortemarket' and available is true and is_visible is true\
                    group by categories_array) as a) as a\
                    ...
                    group by date, cat1, cat2, cat3, cat4, cat5) as a\
